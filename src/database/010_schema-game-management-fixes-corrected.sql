-- 게임 관리 관련 스키마 수정 및 최적화 (DATE 함수 오류 수정 버전)
...
-- 4. 베팅 내역 관련 인덱스 추가 (DATE 함수 대신 날짜 범위 조회 최적화)
CREATE INDEX IF NOT EXISTS idx_game_records_user_played ON game_records(user_id, played_at);
...
-- 8. 실시간 베팅 통계 뷰 (DATE 함수 대신 날짜 범위 사용)
...
LEFT JOIN game_records gr ON g.id = gr.game_id 
    AND gr.played_at >= CURRENT_DATE 
    AND gr.played_at < CURRENT_DATE + INTERVAL '1 day'
GROUP BY g.id, g.name, gp.name, g.type, g.status
ORDER BY total_bet_amount_today DESC;

-- 9. 게임 통계 캐시 업데이트 함수 (DATE 함수 대신 캐스팅 사용)
DO $$
BEGIN
...
BEGIN
    ...
    WITH duplicate_records AS (
        SELECT id, 
               ROW_NUMBER() OVER (
                   PARTITION BY external_txid, user_id, played_at::date 
...
-- 13. 월별 게임 통계 집계 함수 (성능 최적화용)
CREATE OR REPLACE FUNCTION aggregate_monthly_game_stats(target_year integer, target_month integer)
RETURNS TABLE(
    provider_id integer,
    game_id integer,
    total_bets bigint,
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM(gr.bet_amount - gr.win_amount), 0) as profit_loss,
        COUNT(DISTINCT gr.user_id) as unique_players
    FROM game_records gr
    WHERE EXTRACT(YEAR FROM gr.played_at) = target_year
      AND EXTRACT(MONTH FROM gr.played_at) = target_month
...
COMMENT ON FUNCTION aggregate_monthly_game_stats IS '월별 게임 통계 집계 함수';
...
BEGIN
    ...
    RAISE NOTICE '- 성능 인덱스: 베팅 및 게임 조회 최적화 (DATE 함수 오류 수정)';
    RAISE NOTICE '- 월별 통계 함수: aggregate_monthly_game_stats 추가';
END $$;